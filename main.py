...
for i in range(1, 16):
    globals()[f"wall{i}"] = pygame.image.load("wall.png")
...
class App():
    ...
    def __init__(self):
        ...
        self.rect.center = self.mouseX/2, self.mouseY/2
        self.main()
    ...
    def collidecheck(self):
        self.collidewin = pygame.Rect.colliderect(self.mouserect, self.winrect)
        for i in range(1, 15):
            self.collide = pygame.Rect.colliderect(self.mouserect, eval("self.wall" + str(i) + "rect"))
            if self.collide:
                self.mouseX = 500
                self.mouseY = 500
            elif self.collidewin:
                self.win()
